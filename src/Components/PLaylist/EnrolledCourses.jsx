...
const EnrolledCourses = () => {
    ...
    const fetchCourses = async () => {
      ...
      try {
        const enrolledResponse = await axios.get(
          `https://learningmodule-dac4fyf9dccpcfh7.centralindia-01.azurewebsites.net/api/Qtech/EnrolledPlaylistIds`,
          { params: { userEmail: mail }, timeout: 15000 }
        );
        ...
        if (enrolledPlaylistIds.length === 0) {
          setEnrolledCourses([]);
          const allPlaylistsResponse = await axios.get(
            `https://learningmodule-dac4fyf9dccpcfh7.centralindia-01.azurewebsites.net/api/Qtech/Playlists`,
            { timeout: 15000 }
          );
          if (!Array.isArray(allPlaylistsResponse.data)) {
            throw new Error('Invalid response format for all playlists');
        ...
        const enrolledCoursePromises = enrolledPlaylistIds.map((id) =>
          axios.get(`https://learningmodule-dac4fyf9dccpcfh7.centralindia-01.azurewebsites.net/api/Qtech/Playlists/${id}`, {
            timeout: 15000,
          })
        );
        ...
        const contentPromises = enrolledPlaylistIds.map((playlistId) =>
          limit(async () => {
            try {
              const [videosResponse, documentsResponse] = await Promise.all([
              ...
              console.log(`Videos for playlist ${playlistId}:`, videosResponse.data);
              console.log(`Documents for playlist ${playlistId}:`, documentsResponse.data);
              ...
              console.error(`Failed to fetch content for playlist ${playlistId}:`, error);
              return { playlistId, videos: [], documents: [], error: error.message };
            }
          })
        );

        const contentResponses = await Promise.allSettled(contentPromises);
        const contentMap = contentResponses.reduce((acc, result) => {
          if (result.status === 'fulfilled' && !result.value.error) {
            acc[result.value.playlistId] = {
        ...
        if (Object.keys(contentMap).length === 0) {
          setError('No content available for enrolled courses.');
        }
        ...
        const allPlaylists = allPlaylistsResponse.data;
        const  setOtherCourses=(allPlaylists.filter((playlist) => !enrolledPlaylistIds.includes(playlist.playlistId)))
        setLoading(false);
      } catch (err) {
        console.error('Detailed Error:', err);
        ...
        if (err.response) {
          ...
          errorMessage = 'No response received from server. Please check your network or backend status.';
        } else if (err.code === 'ECONNABORTED') {
          errorMessage = 'Request timed out. The server is slow or unreachable.';
        } else {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const [ videosResponse, documentsResponse ] = await Promise.all([
        axios.get(`https://learningmodule-dac4fyf9dccpcfh7.centralindia-01.azurewebsites.net/api/Qtech/Videos`, { params: { playlistId: playlistId }, timeout: 15000 }),
        axios.get(`https://learningmodule-dac4fyf9dccpcfh7.centralindia-01.azurewebsites.net/api/Qtech/Documents`, { params: { playlistId: playlistId }, timeout: 15000 }),
      ]);

      setEnrolledContent((prevContent) => ({
        ...prevContent,
        [playlistId]: {
          videos: videosResponse.data,
          documents: documentsResponse.data,
      ...
      setEnrolledCourses((prevCourses) => [...prevCourses, courseToEnroll]);
      setOtherCourses((prevCourses) => prevCourses.filter((course) => course.playlistId !== playlistId));
      setShowModal(false);
      setCourseToEnroll(null);
    } catch (error) {
